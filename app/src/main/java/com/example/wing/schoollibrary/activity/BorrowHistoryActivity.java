...
import com.example.wing.schoollibrary.adapter.BorrowHistoryAdapter;
...
public class BorrowHistoryActivity extends Activity {
    ...
    private List<BorrowHistory> items;
    ...
    private void getDataFromNet() {
        if(checkForLogin()){
            ...
            OkHttpUtils.get(Urls.URL_BORROWHISTORY)
    ...
    private void showData(List<BorrowHistory> items) {
        if (items!=null&&items.size()>0){
            adapter=new BorrowHistoryAdapter(this,items);
            search_history_listview.setAdapter(adapter);
            tv_nohistory.setVisibility(View.GONE);
        }else{
            tv_nohistory.setVisibility(View.VISIBLE);
            tv_nohistory.setText("没有借书历史");
        }
    }

    private List<BorrowHistory> parseJson(String s) {
        List<BorrowHistory> items=new ArrayList<>();
        Gson gson=new Gson();
        ...
        if (msg.getStatusCode()==100){
            ...
            items=gson.fromJson(json,new TypeToken<List<BorrowHistory>>(){}.getType());
...
//            startActivity(new Intent(this,LoginActivity.class));
            startActivityForResult(new Intent(this,LoginActivity.class),LoginActivity.REQUEST_BORROWHISTORY_CODE);
            return false;
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode==LoginActivity.REQUEST_BORROWHISTORY_CODE&&resultCode!=RESULT_CANCELED&&
